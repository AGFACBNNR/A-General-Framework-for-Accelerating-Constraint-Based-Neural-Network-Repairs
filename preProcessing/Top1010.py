...
if __name__ == "__main__":
    ...
    pred = np.argmax( model.predict(sampleSet),axis=1)
    ...
    for i,sample in enumerate(sampleSet):
        ...
        if (pred[i] == labelSet[i] and matrix[i] <= thresholdCU and matrix[i] >= thresholdCL and cntP < int(len(matrixC)*args.positiveRadio) ):
            cntP += 1
        elif  (pred[i] != labelSet[i] and matrix[i] <= thresholdWU and matrix[i] >= thresholdWL and cntN < int(len(matrixW)*args.negativeRadio) ) : # or pred[i] == labelSet[i]: 
            cntN += 1
        else:
            continue
    ...
    pred = np.argmax(model.predict(filteredSamples),axis=1)
