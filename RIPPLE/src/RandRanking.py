...
parser.add_argument("-m","--method",default= "pd")
...
w = Complete_Model.get_layer(index=-2).get_weights()[0]
b = Complete_Model.get_layer(index=-2).get_weights()[1]
w = np.concatenate((w,b.reshape(1,-1)))
newModel = Model(inputs=Complete_Model.input, outputs=Complete_Model.layers[-3].output)

#load_data
x_train = np.load(args.positive_examples).reshape((-1,28,28,1))
...
#matrix = sort(Complete_Model,x_train,args.method)
#thresholdL = sorted(matrix)[int(len(matrix)*0.5)]
#thresholdR = sorted(matrix)[int(len(matrix)*0.6)]


x_buggy = np.load(args.buggy_examples).reshape((-1,28,28,1))
...
#pred = np.argmax(Complete_Model.predict(),axis=1)
#matrix = sort(Complete_Model,x_buggy,args.method)
#thresholdL = sorted(matrix)[-int(len(matrix)*args.threshold)]
#Split
...
for i in range(len(pred_buggy)):
    ...
    if pred_buggy[i] != y_buggy[i]: #and matrix[i]>=thresholdL:
        negData.append(f_buggy[i]/norm)
        negLabels.append(y_buggy[i])
negLen = len(negLabels)
negInd = sorted(np.random.choice(range(negLen),int(args.threshold*negLen),replace=False))
negData = np.array(negData)[negInd]
negLabels = np.array(negLabels)[negInd]
negLen = len(negLabels)
...
posLen = 0
matrix = sort(Complete_Model,x_train,args.method)
thresholdR = sorted(matrix)[int(len(matrix)*args.threshold)-1]
for i in range(len(pred_train)):
	norm = np.linalg.norm(f_train[i])
	if pred_train[i] == y_train[i] and matrix[i] <= thresholdR:
		posLen += 1
		posData.append(f_train[i]/norm)
		posLabels.append(y_train[i])

#posInd = sorted(np.random.choice(range(posLen),int(args.threshold*posLen),replace=False))
#posData = np.array(posData)[posInd]
#posLabels = np.array(posLabels)[posInd]
